# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 23:14:10 2020
...
# scripts to generate spectrogram
def get_specgram(lfptrace, Fs):
    freqs, time, Pxx =  scsig.spectrogram(lfptrace, fs=Fs, window=('hann'), nperseg=Fs, noverlap=Fs//2)
    return Pxx, freqs, time
...
def get_spectrogram(data, fs, freq_band=(0,250), norm=True, interp=True):
    f, t, Sxx = scsig.stft(data, fs, nperseg=2*fs, noverlap=1.5*fs)
...
def window_rms(a, window_size=10):
    a2 = np.power(a,2)
...
def plot_only_specgram(f, t, P):
    _extent = [t[0], t[-1], min(f), max(f)]
    plt.imshow(P,aspect='auto',extent=_extent,origin='lower', cmap='Spectral_r')
